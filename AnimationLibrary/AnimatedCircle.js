var AnimatedCircle = function(objectID, objectLabel)
{
	...
	this.radius = 36;  // Increased from 26 to 36 for better mobile visibility
	this.thickness = 3;
	this.x = 0;
	this.y = 0;
	this.alpha = 1.0;
...
AnimatedCircle.prototype.draw = function(ctx)
{
	...
	ctx.font         = '16px sans-serif';  // Increased from 12px to 16px for better readability
	...
	else if (strList.length % 2 == 0)
	{
		...
		{
			ctx.fillText(strList[mid - i - 1], this.x, this.y - (i + 0.5) * 18);  // Increased from 12 to 18 for better spacing
			ctx.fillText(strList[mid + i], this.x, this.y + (i + 0.5) * 18);  // Increased from 12 to 18 for better spacing
	...
	else
	{
		...
		for (i = 0; i < mid; i++)
		{
			ctx.fillText(strList[mid - (i + 1)], this.x, this.y - (i + 1) * 18);  // Increased from 12 to 18 for better spacing
			ctx.fillText(strList[mid + (i + 1)], this.x, this.y + (i + 1) * 18);  // Increased from 12 to 18 for better spacing			
		}
